...
W1 = tf.Variable(tf.ones([1, n_dims]), name="weight")
b1 = tf.Variable(tf.zeros([1, n_dims]), name="bias")

layer1 = tf.add(tf.multiply(X, W1), b1)

W = tf.Variable(tf.ones([1, n_dims]), name="weight")
b = tf.Variable(tf.zeros([1, n_dims]), name="bias")

pred = tf.reduce_sum(tf.add(tf.multiply(layer1, W), b), 1)
